...
class Treap {
 public:
  struct Node {
    ...
    Node(T v, int p) : val(v), pri(p), sz(1), sum(v) { left = right = NULL; }
  };
  ...
  Node *find(Node *t, int k) {
    if (!t) return t;
    int sz = size(t->left);
    return k < sz ? find(t->left, k)
                  : k == sz ? t : find(t->right, k - (sz + 1));
  }

  // val 未満の要素の個数
  int count(Node *t, T val) {
    if (!t) return 0;
    if (t->val < val) return size(t->left) + 1 + count(t->right, val);
