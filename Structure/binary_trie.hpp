...
// verified: https://atcoder.jp/contests/arc033/submissions/4648138
class BinaryTrie
{
    using T = unsigned;
    // bit 数
  ...
  public:
    ...
    // 集合内の最大値
    T maximumElement(T mask = 0) const
    {
        return getMinimum(root, ~mask);
    }
    // 集合内の最小値
    T minimumElement(T mask = 0) const
    {
        return getMinimum(root, mask);
    }
    // 集合内で val 以上の要素の最小index
    int lowerBound(T val) const
    {
        return countLessThan(root, val);
    }
    // 集合内で val より大きい要素の最小index
    int upperBound(T val) const
    {
